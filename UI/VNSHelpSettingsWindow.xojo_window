...
Begin DesktopWindow VNSHelpSettingsWindow
   ...
   Begin DesktopLabel DeprecatedExplanation
      ...
      Width           =   392
   End
   Begin DesktopLabel LaunchHeading
      AllowAutoDeactivate=   True
      Bold            =   True
      Enabled         =   True
      FontSize        =   13.0
      ...
      Width           =   412
   End
   Begin DesktopCheckBox ChkRestoreLastPage
      AllowAutoDeactivate=   True
      Caption         =   ""
      Enabled         =   True
      Height          =   20
      ...
      Width           =   412
   End
   Begin DesktopLabel RestoreExplanation
      AllowAutoDeactivate=   True
      Bold            =   False
      Enabled         =   True
      FontSize        =   11.0
...
#tag EndDesktopWindow
	...
	#tag Event
		Sub Opening()
		  ...
		  LaunchHeading.Text = kLaunchHeading
		  ChkRestoreLastPage.Caption = kRestoreCaption
		  RestoreExplanation.Text = kRestoreExplanation
		  ChkRestoreLastPage.Value = VNSHelpPreferences.RestoreLastPage
	...
	#tag Constant, Name = kLaunchHeading, Type = String, Dynamic = False, Default = \"At launch", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kRestoreCaption, Type = String, Dynamic = False, Default = \"Reopen the last page read", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kRestoreExplanation, Type = String, Dynamic = False, Default = \"Turn this off to start every launch fresh: the newest documentation set\x2C opened on its first topic.", Scope = Private
	#tag EndConstant
	...
	#tag Note, Name = Version
		...
		19:29  [NEW] An "At launch" section: whether the last page read reopens on launch (the default) or every launch starts fresh on the newest set's first topic. Applied at the next launch, so its ValueChanged only saves. The MCP section moved down to make room.
...
#tag Events ChkDeprecatedLast
	...
	#tag EndEvent
#tag EndEvents
#tag Events ChkRestoreLastPage
	#tag Event
		Sub ValueChanged()
		  // Takes effect at the next launch, so unlike the boxes around it there is
		  // nothing to reapply now.
		  VNSHelpPreferences.RestoreLastPage = Me.Value
		  VNSHelpPreferences.Save
		End Sub
	#tag EndEvent
#tag EndEvents
