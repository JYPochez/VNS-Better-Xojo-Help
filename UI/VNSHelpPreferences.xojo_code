...
Protected Module VNSHelpPreferences
	#tag Method, Flags = &h0
		Sub Load()
		  ...
		  Try
		    ...
		    mRestoreLastPage = BooleanValue(settings, kKeyRestoreLastPage, mRestoreLastPage)
		    PlatformFilter = IntegerValue(settings, kKeyPlatformFilter, mPlatformFilter)
		    mMCPEnabled = BooleanValue(settings, kKeyMCPEnabled, mMCPEnabled)
		    MCPPort = IntegerValue(settings, kKeyMCPPort, mMCPPort)
	...
	#tag Method, Flags = &h0
		Sub Save()
		  ...
		  settings.Value(kKeyRestoreLastPage) = mRestoreLastPage
		  settings.Value(kKeyPlatformFilter) = mPlatformFilter
		  settings.Value(kKeyMCPEnabled) = mMCPEnabled
		  settings.Value(kKeyMCPPort) = mMCPPort
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  EnsureLoaded
			  Return mRestoreLastPage
			End Get
		#tag EndGetter
		#tag Setter
			Set
			  mRestoreLastPage = value
			End Set
		#tag EndSetter
		RestoreLastPage As Boolean
	#tag EndComputedProperty
	...
	#tag ComputedProperty, Flags = &h0
		...
		#tag Setter
			Set
			  // Out of range falls back to the default rather than being clamped to an
			  // edge: a hand-edited 0 or 99999 is a mistake, and 8722 is more useful
			  // than 1024. Ports below 1024 need root, so they are refused too.
			  If value < kMinimumPort Or value > kMaximumPort Then
			    mMCPPort = VNSHelpMCPServer.kDefaultPort
			  Else
			    mMCPPort = value
			  End If
			End Set
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  // Stored as the bitmask itself rather than seven booleans: it is what
			  // VNSHelpDocSet.PlatformFilter takes, so nothing has to be reassembled.
			  EnsureLoaded
			  Return mPlatformFilter
			End Get
		#tag EndGetter
		#tag Setter
			Set
			  // A preferences file that has been hand-edited, or written by a future
			  // version with more platforms, must not be able to hide everything. Out
			  // of range falls back to all-on, which is the same as not filtering.
			  If value <= 0 Or value > VNSHelpDocSet.kPlatformAll Then
			    mPlatformFilter = VNSHelpDocSet.kPlatformAll
			  Else
			    mPlatformFilter = value
			  End If
			End Set
		#tag EndSetter
		PlatformFilter As Integer
	#tag EndComputedProperty
	...
	#tag Property, Flags = &h21
		Private mRestoreLastPage As Boolean = True
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mPlatformFilter As Integer = VNSHelpDocSet.kPlatformAll
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mMCPEnabled As Boolean = False
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mMCPPort As Integer = VNSHelpMCPServer.kDefaultPort
	#tag EndProperty
	...
	#tag Constant, Name = kKeyMCPEnabled, Type = String, Dynamic = False, Default = \"mcpEnabled", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kKeyMCPPort, Type = String, Dynamic = False, Default = \"mcpPort", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMinimumPort, Type = Integer, Dynamic = False, Default = \"1024", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMaximumPort, Type = Integer, Dynamic = False, Default = \"65535", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kKeyPlatformFilter, Type = String, Dynamic = False, Default = \"platformFilterV2", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kKeyDeprecatedLast, Type = String, Dynamic = False, Default = \"deprecatedLast", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kKeyRestoreLastPage, Type = String, Dynamic = False, Default = \"restoreLastPage", Scope = Private
	#tag EndConstant
	...
	#tag Note, Name = Version
		Version: 0.19.0
		Last change: 2026-08-03 19:29

		------------------------------------------------------------
		0.19.0 — 2026-08-03

		19:29  [NEW] RestoreLastPage. True (the default) keeps today's behaviour — the last page read reopens at launch; False starts every launch fresh. Read by the main window at startup, set from the Settings window.
