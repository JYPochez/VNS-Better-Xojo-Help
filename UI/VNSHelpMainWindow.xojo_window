...
#tag WindowCode
	#tag Event
		Sub Opening()
		  ...
		  // Tab, strip, geometry — then content. All three have to precede LoadVersions,
		  // and the reason is the second bug this ordering caused.
		  //
		  // LoadVersions renders into the viewer when it finds no documentation, and it
		  // used to run first: the page was loaded while the viewer still had the size
		  // the .xojo_window gives it, 610x560 at 278,70. macOS reflows the WebKit view
		  // when Layout moves it afterwards. Linux does not — the widget stayed painted
		  // at the designed rectangle and covered most of the window.
		  //
		  // EnsureFirstTab first because SyncTabBar and PositionViewers read mTabs;
		  // LoadVersions only creates a tab on the path where it finds documentation.
		  EnsureFirstTab
		  SyncTabBar
		  Layout
		  ...
		  // Last, so a listener is never left running if anything above raised.
		  ApplyMCPSettings
	...
	#tag Event
		Sub Closing()
		  ...
		  // Close the listener explicitly. Quitting would release the port anyway, but
		  // not before any connection still open has been dropped, and leaving a
		  // half-closed socket behind is how the next launch finds its port taken.
		  Timer.CancelCallLater(AddressOf RefreshMCPMeter)
		  If mMCPServer <> Nil Then mMCPServer.Stop
	...
	#tag MenuHandler
		Function FileInstallDocs() As Boolean Handles FileInstallDocs.Action
		  // Named after the menu item: a handler renamed to anything else keeps its
		  // Handles clause and silently never fires.
	...
	#tag Method, Flags = &h21
		Private Sub InstallDocumentation()
		  // For someone who no longer has the IDE: take a XojoLangRefDB or a docs.tgz
		  // and put it where this app looks, under its own folder.
		  Var dialog As New OpenFileDialog
		  dialog.Title = kInstallTitle
		  dialog.PromptText = kInstallPrompt

		  Var chosen As FolderItem = dialog.ShowModal
		  If chosen = Nil Then Return

		  // Identification is best-effort by design: a 2020-era database carries no
		  // version and neither do the five oldest Sphinx sets, so an empty answer is
		  // expected rather than exceptional.
		  Var release As String = VNSHelpDocInstaller.Identify(chosen)

		  // Always confirm, even when detected. A wrong release is invisible until the
		  // popup shows the wrong number, and the file's own date is the only other clue
		  // the user has.
		  //
		  // A window rather than InputBox: API 2 has no input dialog — InputBox is gone
		  // and MessageDialog carries no text field.
		  Var prompt As New VNSHelpReleasePromptWindow
		  prompt.Prompt = ReleasePrompt(chosen, release)
		  prompt.ReleaseValue = release
		  prompt.ShowModal

		  If Not prompt.Accepted Then Return

		  Var asked As String = prompt.ReleaseValue.Trim
		  If asked = "" Then Return

		  // Ask before doing the work, not after. Unpacking a docs.tgz is 190 MB through
		  // tar, and finding out at the end that the release was already there wastes
		  // the wait and tells the user nothing they could have acted on.
		  If Not ConfirmOverExisting(asked) Then Return

		  Var failure As String
		  If Not VNSHelpDocInstaller.Install(chosen, asked, failure) Then
		    MessageBox(failure)
		    Return
		  End If

		  // Rescan so it appears without a relaunch, and select what was just added.
		  LoadVersions
		  SelectRelease(asked.Trim)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Function ConfirmOverExisting(release As String) As Boolean
		  // Three cases, and they are not the same:
		  //
		  //   already here      — this app installed it; offer to replace it.
		  //   Xojo has it       — leave that alone entirely and say so. Installing anyway
		  //                       is allowed, and ours then wins the scanner's dedup.
		  //   not present       — nothing to ask.
		  If VNSHelpDocInstaller.IsInstalledHere(release) Then
		    Var replaceIt As New MessageDialog
		    replaceIt.Message = kExistsHereMessage + release
		    replaceIt.Explanation = kExistsHereExplanation
		    replaceIt.ActionButton.Caption = kReplaceCaption
		    replaceIt.CancelButton.Visible = True

		    If replaceIt.ShowModal <> replaceIt.ActionButton Then Return False

		    Var failure As String
		    If Not VNSHelpDocInstaller.RemoveInstalled(release, failure) Then
		      MessageBox(failure)
		      Return False
		    End If
		  ...
		  If Not AlreadyAvailable(release) Then Return True

		  Var anyway As New MessageDialog
		  anyway.Message = kExistsInXojoMessage + release
		  anyway.Explanation = kExistsInXojoExplanation
		  anyway.ActionButton.Caption = kInstallAnywayCaption
		  anyway.CancelButton.Visible = True

		  Return anyway.ShowModal = anyway.ActionButton
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Function AlreadyAvailable(release As String) As Boolean
		  // Whether the popup already offers that release, from any root.
		  For Each v As VNSHelpVersion In mVersions
		    If v <> Nil And v.DisplayName = release Then Return True
		  Next
	...
	#tag Method, Flags = &h21
		Private Function ReleasePrompt(chosen As FolderItem, detected As String) As String
		  If detected <> "" Then Return kPromptDetected + detected

		  // No version in the file. The modification date is a real clue — a database
		  // dated 2020-12-14 is a 2020r2.x — so offer it rather than asking blind.
		  Var dated As String = ""
		  If chosen.ModificationDateTime <> Nil Then dated = kPromptDated + chosen.ModificationDateTime.SQLDate

		  Return kPromptUnknown + dated
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub SelectRelease(release As String)
		  // Pick a release in the popup by name, so a freshly installed set is what the
		  // user is looking at rather than something they have to go and find.
		  For row As Integer = 0 To VersionPopup.LastRowIndex
		    Var tag As Variant = VersionPopup.RowTagAt(row)
		    If tag = Nil Then Continue

		    Var index As Integer = tag.IntegerValue
		    If index < 0 Or index > mVersions.LastIndex Then Continue
		    If mVersions(index).DisplayName <> release Then Continue
	...
	#tag MenuHandler
		Function HelpMCPSetup() As Boolean Handles HelpMCPSetup.Action
		  // Named after the menu item, not for style: a handler renamed to anything
		  // else keeps its Handles clause and silently never fires. See CLAUDE.md.
		  VNSHelpMCPSetupWindow.Show
	...
	#tag Method, Flags = &h0
		Function MCPListening() As Boolean
		  // What the server is doing, which is not the same as what the preference
		  // says: the settings window uses this to avoid claiming a port is being
		  // served when the listener never came up.
		  If mMCPServer = Nil Then Return False

		  Return mMCPServer.IsListening
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Function MCPErrorCode() As Integer
		  // Why the listener is not up, for the settings window to put into words.
		  If mMCPServer = Nil Then Return 0

		  Return mMCPServer.LastErrorCode
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub ApplyMCPSettings()
		  // Idempotent: called from Opening and again from every settings change, so
		  // it has to be safe to run when nothing has changed.
		  ...
		  Var wanted As Integer = VNSHelpPreferences.MCPPort

		  // A port change means stop and start again, not just start: ServerSocket
		  // keeps listening on the old one otherwise, and the user would be told the
		  // new port was live while the old one still answered.
		  If mMCPServer <> Nil And mMCPServer.IsListening And mMCPServer.Port <> wanted Then
		    mMCPServer.Stop
		  End If

		  If mMCPServer = Nil Then mMCPServer = New VNSHelpMCPServer
		  If mMCPServer.IsListening Then Return

		  // The bridge reads documentation itself rather than borrowing the window's
		  // doc set: a tool call can name any installed release, and the one on screen
		  // is only the default.
		  If mMCPBridge = Nil Then
		    Var bridge As New VNSHelpDocBridge
		    bridge.Versions = mVersions
		    mMCPBridge = bridge
		  End If

		  Call mMCPServer.Start(wanted, mMCPBridge)

		  // One poller at a time: CancelCallLater first, or switching the server off
		  // and on again would leave two running and double the refresh rate.
		  Timer.CancelCallLater(AddressOf RefreshMCPMeter)
		  Timer.CallLater(kMeterInterval, AddressOf RefreshMCPMeter)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub ReapplySettings()
		  ...
		  //
		  // The MCP server first, and before the Nil guard: it does not depend on a doc
		  // set being loaded, and switching it on must work even on a machine where no
	...
	#tag Method, Flags = &h21
		Private Sub ShowMCPMeter()
		  // The dot lives in the frame's Paint, so the frame has to be told as well as
		  // the label — otherwise the colour lags a stop or start by up to a poll.
		  MCPFrame.Refresh
		  // Reads the listener, not the preference: the two disagree exactly when it
		  // matters, and a meter claiming to serve a port nothing is bound to would be
		  // worse than no meter.
		  If Not MCPListening Then
		    MCPMeter.Text = kMeterOff
		    Return
		  End If

		  If mMCPBridge = Nil Or mMCPBridge.Requests = 0 Then
		    MCPMeter.Text = kMeterIdle + VNSHelpPreferences.MCPPort.ToString
		    Return
		  End If

		  // Tokens are an estimate and are labelled as one. Characters are what we can
		  // actually count; a model's tokeniser is its own business, and four
		  // characters to a token is the usual rough figure for English prose.
		  MCPMeter.Text = kMeterPrefix + mMCPBridge.Requests.ToString + kMeterCalls _
		    + kMeterApprox + FormatTokens(mMCPBridge.EstimatedTokensIn) + kMeterIn _
		    + FormatTokens(mMCPBridge.EstimatedTokensOut) + kMeterOut
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Function FormatTokens(count As Integer) As String
		  // Thousands as "12k", so the row does not shuffle sideways as the numbers grow.
		  If count < kThousand Then Return count.ToString

		  // Assigned first: Xojo cannot call a method on a parenthesised expression.
		  Var thousands As Integer = count \ kThousand
		  Return thousands.ToString + kThousandSuffix
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub RefreshMCPMeter()
		  // Polled rather than pushed. The bridge lives in Engine and must not reach
		  // into the window, and a request arriving is not worth an observer for a
		  // label that only has to look roughly current.
		  ShowMCPMeter

		  If MCPListening Then
		    Timer.CallLater(kMeterInterval, AddressOf RefreshMCPMeter)
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub LayoutPlatformRow(m As Integer)
		  ...
		  // Third group, right of the platform ones and framed the same way so it reads
		  // as another section of the row rather than as loose text.
		  Var meterStart As Integer = systemsEnd + kCheckGroupGap + kFramePadding
		  MCPMeter.Left = meterStart + kDotColumn
		  MCPMeter.Top = top
		  MCPMeter.Width = kMeterWidth - kDotColumn
		  MCPMeter.Height = kCheckHeight
		  FrameGroup(MCPFrame, meterStart, meterStart + kMeterWidth, top)
	...
	#tag Method, Flags = &h21
		Private Sub LoadVersions()
		  ...
		  If mVersions.LastIndex < 0 Then
		    ...
		    ShowMessage(kNoVersionsTitle, NoVersionsDetail)
		    Return
		  End If
	...
	#tag Method, Flags = &h21
		Private Sub PositionViewers()
		  ...
		  For i As Integer = 0 To mTabs.LastIndex
		    ...
		    // Left/Top/Width/Height are set and read back correctly on every platform.
		    // On Linux GTK discards the origin and draws the widget at the content
		    // origin instead — measured, and not something this code can express its
		    // way out of. See P14-11; nothing here is to be "fixed" for it.
		    v.Left = mViewerLeft
		    v.Top = mViewerTop
	...
	#tag Method, Flags = &h21
		Private Sub SyncTabBar()
		  // Always visible, even for a single tab. It was hidden below two, which meant
		  // the viewer jumped down the moment a second tab opened and back up when it
		  // closed. A strip that is simply always there is steadier to read, and it
		  // keeps the "+" reachable without a Cmd-click first.
		  Var wanted As Boolean = True
	...
	#tag Method, Flags = &h21
		Private Function CloseZoneAt(x As Integer) As Integer
		  ...
		  // wide enough to have drawn one — and when there is more than one tab, so the
		  // hit zone matches what PaintTabBar actually drew.
		  Var tabW As Integer = TabWidth
		  If tabW < kTabCloseMinWidth Or mTabs.Count < 2 Then Return -1
	...
	#tag Method, Flags = &h21
		Private Sub PaintTabBar(g As Graphics)
		  ...
		  Var tabW As Integer = TabWidth
		  // No cross on a lone tab: closing it would leave no tab at all, and CloseTab
		  // already refuses. Drawing a control that declines to work is worse than not
		  // drawing it.
		  Var showClose As Boolean = tabW >= kTabCloseMinWidth And mTabs.Count > 1
	...
	#tag Method, Flags = &h21
		Private Function NoVersionsDetail() As String
		  // The searched paths come from the scanner, so the message cannot name a
		  // folder the scanner does not look in. The old text hardcoded the macOS one
		  // and was therefore simply wrong on Windows and Linux.
		  Return kNoVersionsPrefix + VNSHelpVersionScanner.SearchedLocations + kNoVersionsSuffix
		End Function
	#tag EndMethod
	...
	#tag Property, Flags = &h21
		Private mMCPServer As VNSHelpMCPServer
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mMCPBridge As VNSHelpMCPBridge
	#tag EndProperty
	...
	#tag Constant, Name = kNoVersionsPrefix, Type = String, Dynamic = False, Default = \"Better Xojo Help reads the offline documentation that ships with each Xojo release. Nothing was found in\x3A ", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kNoVersionsSuffix, Type = String, Dynamic = False, Default = \"</p><p>In Xojo\x2C run Preferences \xE2\x96\xB8 General \xE2\x96\xB8 Documentation \xE2\x96\xB8 Install Local Documentation\x2C once per release\x2C then reopen this window.", Scope = Private
	#tag EndConstant
	...
	#tag Constant, Name = kExistsHereMessage, Type = String, Dynamic = False, Default = \"Documentation is already installed for ", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kExistsHereExplanation, Type = String, Dynamic = False, Default = \"Replacing it deletes the copy in this app's own folder and installs the file you chose. Your Xojo installation is not touched.", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kReplaceCaption, Type = String, Dynamic = False, Default = \"Replace", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kExistsInXojoMessage, Type = String, Dynamic = False, Default = \"That release is already available: ", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kExistsInXojoExplanation, Type = String, Dynamic = False, Default = \"It was found in your Xojo installation\x2C which this app only ever reads. You can install this copy as well\x2C and it will be the one used.", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kInstallAnywayCaption, Type = String, Dynamic = False, Default = \"Install Anyway", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kInstallTitle, Type = String, Dynamic = False, Default = \"Install Documentation", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kInstallPrompt, Type = String, Dynamic = False, Default = \"Choose a XojoLangRefDB file\x2C or a docs.tgz from inside a Xojo application bundle.", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kPromptDetected, Type = String, Dynamic = False, Default = \"This documentation is for Xojo release:", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kPromptUnknown, Type = String, Dynamic = False, Default = \"This file does not say which Xojo release it is for. Enter it\x2C for example 2020r2.1", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kPromptDated, Type = String, Dynamic = False, Default = \"

The file was last changed on ", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterOff, Type = String, Dynamic = False, Default = \"MCP not running", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterIdle, Type = String, Dynamic = False, Default = \"MCP on port ", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterPrefix, Type = String, Dynamic = False, Default = \"MCP ", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterCalls, Type = String, Dynamic = False, Default = \" calls", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterApprox, Type = String, Dynamic = False, Default = \"  \xE2\x89\x88", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterIn, Type = String, Dynamic = False, Default = \" in / ", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterOut, Type = String, Dynamic = False, Default = \" out", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kThousandSuffix, Type = String, Dynamic = False, Default = \"k", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kDotSize, Type = Integer, Dynamic = False, Default = \"9", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kDotInset, Type = Integer, Dynamic = False, Default = \"8", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kDotColumn, Type = Integer, Dynamic = False, Default = \"21", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterWidth, Type = Integer, Dynamic = False, Default = \"228", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kMeterInterval, Type = Integer, Dynamic = False, Default = \"1500", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kThousand, Type = Integer, Dynamic = False, Default = \"1000", Scope = Private
	#tag EndConstant
	...
	#tag Constant, Name = kDotOnColor, Type = Color, Dynamic = False, Default = \"&c34C759", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kDotOffColor, Type = Color, Dynamic = False, Default = \"&cFF3B30", Scope = Private
	#tag EndConstant
...
#tag Events MCPFrame
	#tag Event
		Sub Paint(g As Graphics, areas() As Rect)
		  #Pragma Unused areas
		  // Same outline as the two platform groups, so the row reads as three
		  // sections of one thing rather than two framed groups and a loose label.
		  g.DrawingColor = kFrameColor
		  g.PenSize = 1
		  g.DrawRoundRectangle(0, 0, g.Width - 1, g.Height - 1, kFrameRadius, kFrameRadius)

		  // The dot says on or off at a glance, which is the question being asked
		  // most of the time; the text beside it answers the follow-up.
		  g.DrawingColor = If(MCPListening, kDotOnColor, kDotOffColor)
		  g.FillOval(kDotInset, (g.Height - kDotSize) / 2, kDotSize, kDotSize)
		End Sub
	#tag EndEvent
#tag EndEvents
