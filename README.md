...
- **Reopens where you left off**, or starts fresh on the newest release's first topic — your choice,
  in Settings.
...
![Settings](screenshots/settings.png?v=3)
...
The app is not macOS-only. Compiled from source it runs on both and finds the documentation Xojo
installed.

![Better Xojo Help on Windows](screenshots/windows.png?v=2)

Windows is confirmed on **Windows 11 on ARM** against 2026r2 — that is the machine it was tested on,
so an x86-64 build is expected to work but has not been run. Everything works there: the version
popup, the platform filters, tabs, and the MCP server, which is simply switched off in this shot.

![Better Xojo Help on Linux](screenshots/linux.png)

Linux is confirmed on **Ubuntu on ARM under XFCE**. Getting there needed two Xojo issues worked
around, both handled by the app itself so there is nothing for you to configure:

- **WebKit hardware acceleration is turned off** at startup. Without it the reading pane draws
  nothing on some hardware — Xojo documents this and recommends exactly that fix.
- **Startup's tail is deferred by one turn of the event loop.** Filling a popup menu and setting its
  selection before the window is fully presented leaves the reading pane drawn at the window's
  top-left corner instead of in its own pane. That is **Xojo issue #81717**, which Xojo has confirmed
  and will fix in a future release; deferring the work is their suggested workaround.
...
- **Windows and Linux** work, built from source: Windows confirmed on Windows 11 on ARM with 2026r2,
  Linux on Ubuntu on ARM under XFCE. No download is provided for either — see below for why.
