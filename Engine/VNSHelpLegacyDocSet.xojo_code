		    ...
		    Try
		      ...
		      While rows <> Nil And Not rows.AfterLastRow
		        ...
		        If title <> "" And Not seen.HasKey(title) And MatchesTerms(title, terms, mode) Then
		          ...
		          hits.Add(New VNSHelpSearchHit(kPagePrefix + title, title, kind, RankFor(title, terms)))
		        End If
		  ...
		  Try
		    ...
		    While rows <> Nil And Not rows.AfterLastRow
		      ...
		      If title <> "" And Not seen.HasKey(title) Then
		        seen.Value(title) = True
		        hits.Add(New VNSHelpSearchHit(kPagePrefix + title, title, kContextFullText, _
		          RankFor(title, terms)))
		      End If
